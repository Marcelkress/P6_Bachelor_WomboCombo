...
public class RFX4_RaycastCollision : MonoBehaviour
{
    ...
    {
        if (canUpdate) {
            if (raycastTarget != null) UpdateRaycast(raycastTarget);
            else UpdateRaycast();
        }
    }
    ...
    public void UpdateRaycast()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out raycastHit, RaycastDistance, CollisionLayerMask)) {
            ProcessRaycastHit(raycastHit);
        }
        if (RealTimeUpdateRaycast)
            canUpdate = true;
    }

    public void UpdateRaycast(GameObject targetedEnemy)
    {
        raycastTarget = targetedEnemy;

        var raycastHits = Physics.RaycastAll(transform.position, transform.forward, RaycastDistance, CollisionLayerMask);
        ...
        {
            if (targetedEnemy == null || IsHitOnTarget(hit.transform, targetedEnemy.transform))
            {
    ...
    private static bool IsHitOnTarget(Transform hitTransform, Transform targetTransform)
    {
        if (hitTransform == null || targetTransform == null) return false;
    ...
    private void ProcessRaycastHit(RaycastHit raycastHit)
    {
            Vector3 position;
            if (UsePivotPosition)
